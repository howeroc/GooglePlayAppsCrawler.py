...
class UpdateApp:
    ...
    def scrape_apps(self):
        ...
        self._mongo_wrapper.ensure_index('LastUpdateDate', self._params['apps_collection'])
        ...
        while True:
            ...
            # Finds an app to be checked need to be update or not
            app_record = self._mongo_wrapper.find_app_and_modify(self._params['apps_collection'])

            if not app_record:
                # When no app to check, update all IsCheckedUpdate to False
                self._mongo_wrapper.update_app_check_status(self._params['apps_collection'])
                continue

            try:
                url = app_record['_id']
                last_update_date = app_record['LastUpdateDate']

                # Do we need to normalize the url ?
                if 'http://' not in url and 'https://' not in url:
                    url = 'https://play.google.com' + url

                self._logger.info('Check App need update or not: %s' % url)
                ...
                try:
                    ...
                    app['IsCheckedUpdate'] = True# is this app checked updateInfo
                    app['IsBusy'] = False# is this app be using

                    # Reaching related apps
                    update_date = app['LastUpdateDate']
                    ...
                    # When the app info updated
                    if update_date > last_update_date:
                        # Inserting data into MongoDB
                        self._mongo_wrapper.update_app(url, app, self._params['apps_collection'])
                        self._logger.info('Updating App info: %s' % url)
